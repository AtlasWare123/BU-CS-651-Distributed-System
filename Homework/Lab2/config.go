...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		leaders := make(map[int][]int) // leaders[term][0] = server number
		for i := 0; i < cfg.n; i++ {   // cfg.n is the number of server
			if cfg.connected[i] { // whether each server is on the net
				if t, leader := cfg.rafts[i].GetState(); leader { // GetState() return an int and a bool, term and check if it's a leader
					leaders[t] = append(leaders[t], i) // if it's a leader, put the ith server into the leaders array.
				}
			}
		}
		...
		if len(leaders) != 0 {
			return leaders[lastTermWithLeader][0] // represent in the term lastTermWithLeader, which server is the leader.
		}
	}
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		// fmt.Printf("index = %d, cmd1 = %d and log = %+v, ok = %v \n", index, cmd1, cfg.logs, ok) // special
		cfg.mu.Unlock()
